...
export function generateAchievements () {
  for (let x = 0; x < achievementsArr.length; x++) {
    const achievementTab = createTagElement('div', `achievement achievement-${x}`, '', achievementsWrapper)
...
function checkifAchievementDone (num, stars) {
  ...
  const goalConvert = convertingNumbers(goal)
  let currentNumber
  if (typeof (current) === 'object') {
    const differencePowerOfTen = current.powerOfTen - goalConvert.powerOfTen
